---
title: "Lab 06 - Take a sad plot and make it better"
author: "John, Angus and Yousuf - R you serious???"
date: "`r Sys.Date()`"
output: html_document
---

### Load packages

```{r load-packages, message = FALSE, warning=FALSE}
library(tidyverse)
library(ggplot2)
```


# Questioning data visualisations

## Image 1

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/Speaking-the-world.jpg")
```

_Write your comments about image 1 below_
The size of bars are really inconsistent Hindi is a much smaller bar than Spanish but the amount of speakers is not as much less. The bar for China is much thicker than the bar for English - over twice the thickness. China's number of speakers is 1,026 Billion but there isn't even 1,026 Billion people in the world, it was definitively meant to be 1,026 Million. The bar for English is also not much larger than the bar for Spanish even though there are many more speakers for English than Spanish. 




## Image 2

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/rose-pop-density-total-area-bw.jpg")
```

_Write your comments about image 2 below_




## Image 3

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/life-expectancy-rule-of-law.png")
```

_Write your comments about image 3 below_




## Image 4

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("img/global-income-dist.PNG")
```

_Write your comments about image 4 below_





# Exercises

```{r load-data-staff, message=FALSE, eval=TRUE}
staff <- read_csv("data/instructional-staff.csv")
```


## Exercise 1

What is the difference between *wide format* data and *long format* data?
Wide format is when there are more columns than rows
long format is when there are more rows than columns

Is the format `staff` data *wide* or *long*?
Wide

How many observations are there in the data set?

5


## Exercise 2

```{r pivoting}
staff_long <- staff %>%
  pivot_longer(
    cols = -faculty_type,
    names_to = "date",
      values_to = "percent"
  )
```


## Exercise 3

```{r dot-plot}
# Write your code for Exercise 3 here
ggplot(data = staff_long,
       mapping = aes(x = date, 
                     y = percent, 
                     colour = faculty_type)) +
  geom_point()


```

## Exercise 4

```{r bad-line-plot}
# Write your code for Exercise 4 here
ggplot(data = staff_long,
       mapping = aes(x = date, 
                     y = percent, 
                     colour = faculty_type)) +
  geom_line()


```

## Exercise 5

```{r mutate-year}
staff_long <- staff_long %>%
  mutate(date = as.numeric(date))
```


```{r working-line-plot}
# Write your code for Exercise 5 here
ggplot(data = staff_long,
       mapping = aes(x = date, 
                     y = percent, 
                     colour = faculty_type)) +
  geom_line()


```

## Exercise 6

```{r better-line-plot}
# Write your code for Exercise 6 here
ggplot(data = staff_long,
       mapping = aes(x = date, 
                     y = percent, 
                     colour = faculty_type)) +
  labs(title = "percentage of faculty type over the years", x= "Year", y= "Percentage of Faculty type"
      )+
  geom_line()



```

## Exercise 7

_Write below your ideas for adapting the line plot_

* Idea 1
* Idea 2
* Idea 3
* ...


## Exercise 8

```{r implement-Ex7}
# Write your code for Exercise 8 here



```

